...
# MLP modeli
mlp = MLPRegressor(hidden_layer_sizes=(100, 100, 100), max_iter=1000, random_state=42, early_stopping=True, n_iter_no_change=10)

# Cross validation
cv_scores = cross_val_score(mlp, X_train, y_train, cv=5, scoring='neg_mean_squared_error')
mean_cv_score = -np.mean(cv_scores)

print(f"Çapraz Doğrulama Ortalama MSE: {mean_cv_score}")
...
# Performans metrikleri raporlama
...
def predict():
    try:
        ...
        prediction_scaled = mlp.predict(user_input_scaled)
